...
### Community content on the drift-rescue paths

When the uploader has to reconcile Commons-side state before landing an item, it never discards community contributions. Under the SHA1-uniqueness redesign (PR C+D) the old Case-2 "upload a second copy and tag the stranded file `{{Duplicate}}`" path is gone; the surviving rescue paths each preserve community content in their own way:

- **Move** (`_move_to_correct_title`, the `MOVED` drift outcome) preserves page content *inherently* — it renames the existing file to the canonical title and does not rewrite the description at all. The community-vs-DPLA reconciliation (importing community edits as provenance-aware SDC, replacing only drifted bot fields) is deferred to the post-SDC `sdc-sync` cleanup (`_post_sdc_cleanup_for_page`), which runs the `legacy_artwork` revision-history walk on the moved file later in the same pipeline.
- **Redirect overwrite** (`_resolve_redirect_overwrite`, on the fresh-upload path when the intended title is a stale redirect) preserves community content via `rescue_wikitext`: it node-swaps only the metadata-template node for our fresh `{{DPLA metadata}}` block and keeps everything else — categories, `{{ImageNote}}` annotations, every community template — verbatim. (`merge_preserved_wikitext` survives only as `rescue_wikitext`'s narrow license/category/assessment fallback for a source page with *no* recognised wrapper to swap — inherent to that no-wrapper case, not pending work.)
- **Source duplication** no longer uploads+tags. It merges this item's SDC onto the earliest existing (canonical) file and leaves a `#REDIRECT` at our intended title (the `MERGE_AND_REDIRECT` outcome), so there is no new page onto which community wikitext would need rescuing.
- **Community files** — a SHA1 match against a file that is BOTH non-DPLA/NARA-shaped in title AND not one of our bots' uploads is never renamed, merged onto, redirected, or migrated; it is handed to a human via the `hand-fix.jsonl` sidecar untouched.

The legacy `{{Artwork}}` / `{{DPLA metadata}}` template-param migration itself (via `ingest_wikimedia/legacy_artwork.py`) is unchanged: it still walks revision history to separate community-authored param values from DPLA-bot values and imports the former as provenance-aware SDC (the `P887 → Q131783016` + `P4656 → <permalink>` reference shape) before rewriting the wikitext. See [special-cases.md](special-cases.md#hash-drift-resolution) for the full set of drift outcomes.
