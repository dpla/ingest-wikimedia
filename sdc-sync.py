...
parser.add_argument('--cat', dest='cat', metavar='CAT',
                    action='store')
parser.add_argument('--method', dest='method', metavar='METHOD',
                    action='store')
parser.add_argument('--lists', dest='lists', metavar='LISTS',
                    action='store')
...
hubs = json.loads(requests.get('https://raw.githubusercontent.com/dpla/ingestion3/develop/src/main/resources/wiki/institutions_v2.json').text)

rights = json.load(open('rights.json'))
subject_ids = json.loads(requests.get('https://raw.githubusercontent.com/DominicBM/ingestion3/develop/src/main/resources/subjects.json').text)
                ...
                "P813": [
                    {
                        ...
                        "datavalue": {
                            "value": {
                                "time": "+" + str(datetime.date.today()) + "T00:00:00Z",
                                "timezone": 0,
                                "before": 0,
                                "after": 0,
                                "precision": 11,
...
def postqual(claimid, prop, value):
    ...
    summary = u'Adding [[:d:Property:' + prop + ']] to ' + claimid + '.'
    ...
    try:
        qual = json.loads(http.fetch(u'https://commons.wikimedia.org/w/api.php', method='POST', data=postdata).text)
...
def check(mediaid, qid, prop):
    ...
    if qid[0] == 'item':
        if any(statement['mainsnak']['datavalue']['value']['id'] == qid[1] and not statement.get('references') for statement in statements):
            for statement in statements:
                if statement['mainsnak']['datavalue']['value']['id'] == qid[1] and not statement.get('references'):
                    ref = statement['id']
        if any(statement['mainsnak']['datavalue']['value']['id'] == qid[1] and not statement.get('qualifiers') for statement in statements):
            for statement in statements:
                if statement['mainsnak']['datavalue']['value']['id'] == qid[1] and not statement.get('qualifiers'):
                    return add_det(statement['id']), ref
                    exit()

        elif any(statement['mainsnak']['datavalue']['value']['id'] == qid[1] for statement in statements):
            print(' -- There already exists a statement with a ' + prop + ' > ' + qid[1] + ' claim for ' + mediaid + '.')
            ...
            return True, ref
    if qid[0] == 'string':
        if any(statement['mainsnak']['datavalue']['value'] == qid[1] and not statement.get('references') for statement in statements):
            for statement in statements:
                if statement['mainsnak']['datavalue']['value'] == qid[1] and not statement.get('references'):
                    ref = statement['id']
        if any(statement['mainsnak']['datavalue']['value'] == qid[1] and not statement.get('qualifiers') for statement in statements):
            for statement in statements:
                if statement['mainsnak']['datavalue']['value'] == qid[1] and not statement.get('qualifiers'):
                    return add_det(statement['id']), ref
                    exit()

        elif any(statement['mainsnak']['datavalue']['value'] == qid[1] for statement in statements):
            print(' -- There already exists a statement with a ' + prop + ' > ' + qid[1] + ' claim for ' + mediaid + '.')
            #         print(existing_data.get('statements'))
            return False, ref
    ...
    if qid[0] == 'monolingualtext':
        if any(statement['mainsnak']['datavalue']['value']['text'] == qid[1] and not statement.get('references') for statement in statements):
            for statement in statements:
                if statement['mainsnak']['datavalue']['value']['text'] == qid[1] and not statement.get('references'):
                    ref = statement['id']
        if any(statement['mainsnak']['datavalue']['value']['text'] == qid[1] and not statement.get('qualifiers') for statement in statements):
            for statement in statements:
                if statement['mainsnak']['datavalue']['value']['text'] == qid[1] and not statement.get('qualifiers'):
                    return add_det(statement['id']), ref
                    exit()

        elif any(statement['mainsnak']['datavalue']['value']['text'] == qid[1] for statement in statements):
            print(' -- There already exists a statement with a ' + prop + ' > ' + qid[1] + ' claim for ' + mediaid + '.')
            #         print(existing_data.get('statements'))
            return False, ref
    ...
    if qid[0] == 'somevalue':
        try:
            if any(statement['qualifiers'].get('P2093') for statement in statements):
                for statement in statements:
                    if statement['qualifiers']['P2093'][0]['datavalue']['value'] == qid[1] and not statement.get('references'):
                        ref = statement['id']
                for statement in statements:
                    try:
                        if statement['qualifiers']['P2093'][0]['datavalue']['value'] == qid[1]:
                            print(' -- There already exists a statement with a ' + prop + ' > ' + qid[1] + ' claim for ' + mediaid + '.')
                            #         print(existing_data.get('statements'))
                            return False, ref
    ...
    if qid[0] == 'source':
        try:
            if any(statement['qualifiers'].get('P973') for statement in statements):
                for statement in statements:
                    if statement['qualifiers']['P973'][0]['datavalue']['value'] == qid[1] and not statement.get('references'):
                        ref = statement['id']
                for statement in statements:
                    try:
                        if statement['qualifiers']['P973'][0]['datavalue']['value'] == qid[1]:
                            print(' -- There already exists a statement with a ' + prop + ' > ' + qid[1] + ' claim for ' + mediaid + '.')
                            #         print(existing_data.get('statements'))
                            return False, ref
...
def add_rs(mediaid, rs, dpla_id):
    if rights.get(rs):
        ...
        qid = rights[rs][prop]
        summary = u' -- Adding [[:d:Property:' + prop + ']] to ' + mediaid + '.'
        claim = formattedclaim(prop, { "entity-type": "item", "numeric-id": int(qid.replace(u'Q', u'')) }, 'wikibase-entityid', dpla_id)
...
def add_title(mediaid, title, dpla_id):
    if title:
        ...
        prop = u'P1476'
        summary = u' -- Adding [[:d:Property:' + prop + ']] to ' + mediaid + '.'
        claim = formattedclaim(prop, {"text": title[:1499].rstrip(), "language": "en"}, 'monolingualtext', dpla_id)
...
def add_desc(mediaid, desc, dpla_id):
    if desc:
        ...
        prop = u'P10358'
        summary = u' -- Adding [[:d:Property:' + prop + ']] to ' + mediaid + '.'
        claim = formattedclaim(prop, {"text": desc[:1499].rstrip(), "language": "en"}, 'monolingualtext', dpla_id)
...
def add_creator(mediaid, creator, dpla_id):
    if creator:
        ...
        claim['qualifiers']['P2093'] = [{"snaktype": "value", "property": "P2093", "datavalue": { "value": creator[:1499].rstrip(), "type": "string"}}]
...
def add_contributed(mediaid, hub, institution, dpla_id):
    ...
    qid = u'Q2944483'
    claim = formattedclaim(prop, { "entity-type": "item", "numeric-id": int(qid.replace(u'Q', u'')) }, 'wikibase-entityid', dpla_id)
    claim['qualifiers']['P3831'] = [{"snaktype": "value", "property": "P3831", "datavalue": { "value": { "entity-type": "item", "numeric-id": 393351 }, "type": "wikibase-entityid" }}]
    checkclaim = check(mediaid, ('item', qid), prop)
    if checkclaim[1]:
        add_ref(checkclaim[1], claim)
    if checkclaim[0] == True:
    ...
    if hub == u'Q518155':
        qid = u'Q518155'
        claim = formattedclaim(prop, { "entity-type": "item", "numeric-id": int(qid.replace(u'Q', u'')) }, 'wikibase-entityid', dpla_id)
        claim['qualifiers']['P3831'] = [{"snaktype": "value", "property": "P3831", "datavalue": { "value": { "entity-type": "item", "numeric-id": 108296843 }, "type": "wikibase-entityid" }}]
        checkclaim = check(mediaid, ('item', qid), prop)
        if checkclaim[1]:
            add_ref(checkclaim[1], claim)
        if checkclaim[0] == True:
        ...
        qid = institution
        claim = formattedclaim(prop, { "entity-type": "item", "numeric-id": int(qid.replace(u'Q', u'')) }, 'wikibase-entityid', dpla_id)
        claim['qualifiers']['P3831'] = [{"snaktype": "value", "property": "P3831", "datavalue": { "value": { "entity-type": "item", "numeric-id": 108296919 }, "type": "wikibase-entityid" }}]
        checkclaim = check(mediaid, ('item', qid), prop)
        if checkclaim[1]:
            add_ref(checkclaim[1], claim)
        if checkclaim[0] == True:
...
def add_local_id(mediaid, id, institution, dpla_id):
    if id:
        ...
        checkclaim = check(mediaid, ('string', id), prop)
        claim['qualifiers']['P195'] = [{"snaktype": "value", "property": "P195", "datavalue": { "value": { "entity-type": "item", "numeric-id": int(institution.replace(u'Q', u'')) }, "type": "wikibase-entityid" }}]
        if checkclaim[1]:
            add_ref(checkclaim[1], claim)
        if checkclaim[0] == True:
            pywikibot.output(summary)
...
def add_source(mediaid, hub, url, dpla_id):
    ...
    summary = u' -- Adding [[:d:Property:' + prop + ']] to ' + mediaid + '.'
    claim = formattedclaim(prop, { "entity-type": "item", "numeric-id": int(qid.replace(u'Q', u'')) }, 'wikibase-entityid', dpla_id)
    claim['qualifiers']['P973'] = [{"snaktype": "value", "property": "P973", "datavalue": { "value": url, "type": "string"}, "datatype": "url"}]
    claim['qualifiers']['P137'] = [{"snaktype": "value", "property": "P137", "datavalue": { "value": { "entity-type": "item", "numeric-id": int(hub.replace(u'Q', u'')) }, "type": "wikibase-entityid" }}]
...
def add_det(claimid):
    if claimid:
        qid = u'Q61848113'
        prop = u'P459'
        value = json.dumps({"entity-type":"item","numeric-id": int(qid.replace(u'Q', u'')) })
...
def dpla_claims(mediaid, dpla_id, url, descs, dates, titles, hub, local_ids, institution, rs, creators, subjects, naids, access, level):
    print(' -- Accessing Commons ID ' + mediaid)
    try:
        file_claims = json.loads(requests.get('https://commons.wikimedia.org/wiki/Special:EntityData/' + mediaid + '.json').text)
    except:
        file_claims = {}
    ...
    props = ['P6216', 'P275', 'P217', 'P6426', 'P760', 'P1476', 'P195', 'P170', 'P9126', 'P7482', 'P4272', 'P571', 'P10358', 'P1225', 'P7228', 'P6224', 'P921']
    claims = {'P6216': statusvalue, rightsprop: rightsvalue, 'P217': local_ids, 'P760': [dpla_id], 'P1476': titles, 'P195': ['Q518155' if hub == 'Q518155' else institution], 'P170': creators, 'P9126': ['Q2944483', hub, institution], 'P7482': [url], 'P4272': subjects, 'P571': dates, 'P10358': descs, 'P1225': naids, 'P6224': [level], 'P7228': [access],  'P921': parsesubjectentities}
    for prop in file_claims['entities'][mediaid]['statements'].keys():
        for stmt in file_claims['entities'][mediaid]['statements'][prop]:
            if stmt.get('references'):
                if any(pub['snaks']['P123'][0]['datavalue']['value']['id'] == 'Q2944483' for pub in stmt['references']):
                    if stmt['mainsnak']['snaktype'] == 'value':
                        type = stmt['mainsnak']['datavalue']['type']
                        if stmt['mainsnak']['property'] == 'P7482':
                            try:
                                dpla_claims.append({stmt['mainsnak']['property']: {"id": stmt['id'], "value": stmt['qualifiers']['P973'][0]['datavalue']['value']} })
                            except:
                                pass
                        elif type == 'wikibase-entityid':
                            dpla_claims.append({stmt['mainsnak']['property']: {"id": stmt['id'], "value": stmt['mainsnak']['datavalue']['value']['id']} })
                        elif type == 'string':
                            dpla_claims.append({stmt['mainsnak']['property']: {"id": stmt['id'], "value": stmt['mainsnak']['datavalue']['value']} })
                        elif type == 'monolingualtext':
                            dpla_claims.append({stmt['mainsnak']['property']: {"id": stmt['id'], "value": stmt['mainsnak']['datavalue']['value']['text']} })
                    if stmt['mainsnak']['snaktype'] == 'somevalue':
                        try:
                            dpla_claims.append({stmt['mainsnak']['property']: {"id": stmt['id'], "value": stmt['qualifiers']['P2093'][0]['datavalue']['value']} })
                        except:
    ...
    for claim in dpla_claims:
        for prop in claim.keys():
            if prop not in claims.keys():
                ...
                removals.append(claim[prop]['id'])
    if len(removals) > 0:
        rmdata = {u'action' : u'wbremoveclaims',
                  u'format' : u'json',
                  u'id' : mediaid,
                  u'claim' : '|'.join(removals),
                  u'token' : token,
                  u'bot' : True,
                  u'summary': u'Changing structured data claims from [[COM:DPLA|DPLA]] item \'[[dpla:' + dpla_id + '|' + dpla_id + ']]\'. [[COM:DPLA/MOD|Leave feedback]]!'
                  }

        save = http.fetch(u'https://commons.wikimedia.org/w/api.php', method='POST', data=rmdata)
...
def parsed(dpla_id):
    ...
    try:
        dpla = json.loads(requests.get('https://api.dp.la/v2/items/' + dpla_id + '?api_key=4e956679e5d18f3acdf8edb6c35f2462', timeout=15).text)
    except:
        print(' -- Sleeping 30 seconds and retrying...')
        time.sleep(30)
        dpla = json.loads(requests.get('https://api.dp.la/v2/items/' + dpla_id + '?api_key=4e956679e5d18f3acdf8edb6c35f2462').text)
    ...
    institution = hubs[dpla['provider']['name']]['institutions'][dpla['dataProvider']['name']]['Wikidata']
    ...
    try:
        ...
        for subject in dpla['sourceResource']['subject']:
            ...
            if subject.get('name') in subject_ids:
                ...
                subjqid = ''
                naid = subject.get('exactMatch')[0].replace('https://catalog.archives.gov/id/', '')
                h = requests.get('https://wikidata.reconci.link/en/api?queries=%7B%0A%20%20%22q1%22%3A%20%7B%0A%20%20%20%20%22query%22%3A%20%22' + str(subject.get('name') or '') + '%22%2C%0A%20%20%20%20%20%20%22limit%22%3A%205%2C%0A%20%20%20%20%20%20%22properties%22%3A%20%5B%0A%20%20%20%20%20%20%20%20%7B%0A%20%20%20%20%20%20%20%20%20%20%22pid%22%3A%20%22P1225%22%2C%0A%20%20%20%20%20%20%20%20%20%20%22v%22%3A%20%22' + naid + '%22%0A%20%20%20%20%20%20%20%20%7D%0A%20%20%20%20%20%20%5D%2C%0A%20%20%20%20%20%20%22type_strict%22%3A%20%22should%22%0A%20%20%20%20%7D%0A%7D')
    ...
    if dpla['provider']['name'] == "National Archives and Records Administration":
        ...
        codes = {"10031403": "Q66739888", "10031402": "Q24238356", "10031399": "Q66739729", "10031400": "Q66739849", "10031401": "Q66739875"}
        levels = {"item": "Q11723795", "itemAv": "Q11723795", "fileUnit": "Q59221146"}
        xml = BeautifulSoup(dpla['originalRecord']['stringValue'], 'xml')
        try:
            acccess_naid = str(xml.find("accessRestriction").find("status").find("naId").text)
    ...
    return url, descs, dates, titles, hub, local_ids, institution, rs, creators, subjects, naids, access, level
    ...
    if len(claims['claims']) > 0:
        posted = True
        try:
            post = json.loads(http.fetch(u'https://commons.wikimedia.org/w/api.php', method='POST', data=postdata).text)
...
if method == 'list':
    ...
    while len(lists) > 0:
        ...
        for file in files:
            ...
            try:
                url, descs, dates, titles, hub, local_ids, institution, rs, creators, subjects, naids, access, level = parsed(dpla_id)
            except TypeError:
            ...
            count = count + 1
            print(str(count) + ':\n - ' + args.lists + '/' + lists[x] + ' (' + str('{:.2f}'.format(percent)) + '% done)')
            ...
            if len(refclaims['claims']) > 0:
                postrefs = {u'action' : u'wbeditentity',
                            u'format' : u'json',
                            u'id' : mediaid,
                            u'data' : json.dumps(refclaims),
                            u'token' : token,
                            u'bot' : True,
                            u'summary': u'Added structured data references from [[COM:DPLA|DPLA]] item \'[[dpla:' + dpla_id + '|' + dpla_id + ']]\'. [[COM:DPLA/MOD|Leave feedback]]!'
                            }
                save = http.fetch(u'https://commons.wikimedia.org/w/api.php', method='POST', data=postrefs)
                try:
            ...
            postdata = {u'action' : u'wbeditentity',
                        u'format' : u'json',
                        u'id' : mediaid,
                        u'data' : json.dumps(claims),
                        u'token' : token,
                        u'bot' : True,
                        u'summary': u'Added structured data claims from [[COM:DPLA|DPLA]] item \'[[dpla:' + dpla_id + '|' + dpla_id + ']]\'. [[COM:DPLA/MOD|Leave feedback]]!'
                        }

            if len(claims['claims']) > 0:
                try:
                    save = http.fetch(u'https://commons.wikimedia.org/w/api.php', method='POST', data=postdata)
            ...
            dpla_claims(mediaid, dpla_id, url, descs, dates, titles, hub, local_ids, institution, rs, creators, subjects, naids, access, level)
