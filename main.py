...
# #
# # # Download
# # downloaded_file, download_time, filesize = wikiutils.download_single_item(url=file_url, save_location=save_location)
# #
# # # Create wiki page title
# # wikiPageTitle = wikiutils.create_wiki_page_title(title=title, dpla_identifier=dpla_identifier, suffix=downloaded_file[-4:])
# #
# # # Create file page in wiki
# # wikiFilePage = wikiutils.create_wiki_file_page(wikiPageTitle)
# #
# # # Upload to wikimedia
# # wikiutils.upload(wiki_file_page=wikiFilePage, dpla_identifier=dpla_identifier, text=wiki_markup, file=downloaded_file)
# #
...
#     status = 'incomplete'
#     try:
#         # wiki_page = wikiutils.create_wiki_file_page(title=row['title'])
#         # wikiutils.upload(wiki_file_page=wikiFilePage, dpla_identifier=dpla_identifier, text=wiki_markup, file=downloaded_file)
#         # wikiutils.upload(wiki_file_page=wiki_page, dpla_identifier=row['id'], text=row['wiki_markup'], file=row['file_out'])
...
#     try:
#         # wikiutils.duownload() will raise an exception if file cannot be downloaded
