...
Eligibility and domain assignment for each institution are determined by inspecting a **single sample item** from that institution. This approach assumes that all items from a given institution are hosted on the same platform and share the same media delivery domain — an assumption that holds for most contributing institutions but may not be true in all cases. An institution whose sample item happens to come from a non-eligible platform could be incorrectly excluded, and one whose items are split across multiple domains will only have one domain represented. These cases are expected to be rare, but the counts and domain groupings should be treated as estimates rather than exact figures.
...
3. For each hub:
   - If `"upload": true` at the hub level, records it as "already participating hub-wide" in the README and skips further processing
   ...
   - If no eligible institutions are found, records "no eligible items" in the README (no inventory file is created; result is cached to avoid re-fetching)
