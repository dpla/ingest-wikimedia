...
class IIIF:
    ...
    def get_iiif_urls(self, iiif):
        ...
        # v2 or v3?
        if manifest.get('@context', None) == "http://iiif.io/api/presentation/3/context.json":
            return self.iiif__v3_urls(manifest)
        elif manifest.get('@context', None) == "http://iiif.io/api/presentation/2/context.json":
    ...
    def _get_iiif_manifest(self, url):
        ...
        try:
            request = requests.get(url, timeout=30, allow_redirects=True, headers=self.HEADERS)
