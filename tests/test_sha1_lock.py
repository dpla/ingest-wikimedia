...
def test_acquire_holds_exclusive_then_release_frees(tmp_path):
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.callback(release_sha1_lock, fd)
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.callback(os.close, probe2)
...
def test_different_sha1_different_bucket_do_not_block(tmp_path):
    ...
    with contextlib.ExitStack() as stack:
        ...
        stack.callback(release_sha1_lock, fd_a)
        fd_b = acquire_sha1_lock(b, lock_dir)  # must not block on fd_a
        stack.callback(release_sha1_lock, fd_b)
        assert fd_a != fd_b
...
def test_release_is_idempotent(tmp_path):
    with contextlib.ExitStack() as stack:
        ...
        stack.callback(release_sha1_lock, fd)  # guaranteed final close
        # A second, explicit release now must be a tolerated no-op; the
        # ExitStack callback then releases the already-closed fd (also a no-op),
        # proving release_sha1_lock tolerates being called more than once.
