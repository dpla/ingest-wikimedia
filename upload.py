...
class Upload:
    ...
    def __init__(self):
        ...
        self.logger.info(f"Logged in user is: {self.site.user()}")
    ...
    def upload(self, wiki_file_page, dpla_identifier, text, file):
        ...
        try:
            ...
            if file.startswith("s3"):
                ...
                self.logger.info(utils.timer_message(msg="Download s3 to tmp", start=start, end=end))
            ...
            self.logger.info(utils.timer_message(msg="Upload to wikimedia", start=start, end=end))
    ...
    def create_wiki_file_page(self, title):
        ...
        try:
            ...
            self.logger.error(f"Unable to create FilePage: {e}")
...
for parquet_file in file_list:
    ...
    for row in df.itertuples(index=columns):
        ...
        try:
            ...
            logger.error(f"Unable to get attributes from row {row}: {e}")
            break
        end = time.process_time()
        logger.info(utils.timer_message(msg="Load record from dataframe", start=start, end=end))
        ...
        logger.info(utils.timer_message(msg="Get file extension", start=start, end=end))
        ...
        logger.info(utils.timer_message(msg="Create wiki page title", start=start, end=end))

        # Create wiki page
        start = time.process_time()
        wiki_page = uploader.create_wiki_file_page(title=page_title)
        end = time.process_time()
        logger.info(utils.timer_message(msg="Create wiki page", start=start, end=end))
        ...
        try:
            upload_status = uploader.upload(
                ...
                file=path)
            if upload_status:
                logger.info(f"Uploaded count {upload_count}")
        ...
        logger.info(utils.timer_message(msg="Upload image", start=start, end=end))
        logger.info(utils.timer_message(msg="Upload image (w/sleep)", start=image_upload_sleep_start, end=end_image))
        logger.info(utils.timer_message(msg="Overall proc time)", start=start_image_proc, end=end))
        logger.info(utils.timer_message(msg="Overall time (w/sleep)", start=start_image, end=end_image))
        logger.info("--------------------------------------------------------------")
